...
CChart::CChart()
{
	...
	memset(nPlotIndex, 0, MAX_SERIES);
	nPointCount = 0 ;
	nSerieCount = 1 ;
...
void CChart::CalcRect()
{
	...
	offsetY   = m_clientRect.Height()/4 ;
	offsetX   = m_clientRect.Width()/5/3 ;
...
void CChart::Plot(CDC *pDC)
{
	...
	for( i = 0 ; i < nSerieCount ; i++) {
		...
		pen = new CPen(PS_SOLID,1,mpSerie[i].m_plotColor);
		old = pDC->SelectObject(pen);
		...
		for (int index = 1; index < nPlotIndex[i]; index++) {
			pt = Corrdinate(mpSerie[i].dValueX[index], mpSerie[i].dValueY[index]);
			pDC->LineTo(pt);
		}
		pDC->SelectObject(old);
		delete pen ;
...
void CChart::DrawChartTitle(CDC *pDC)
{
	...
	int oldbkmode ;
	CFont *old ; 
	COLORREF oldForeColor = pDC->SetTextColor(RGB(255, 255, 255));
...
BOOL CChart::SetXYValue(double x, double y, int index, int iSerieIdx)
{
	...
	nPlotIndex[iSerieIdx] = index + 1 ;
	return TRUE ;

}

BOOL CChart::AddYValue(double y, int iSerieIdx)
{
	if (!bIsSerieAllocated) return FALSE;
	if (iSerieIdx > (nSerieCount - 1)) return FALSE;

	// Prevent writting to unallocated buffer 
	if (nPlotIndex[iSerieIdx] >= nPointCount)
	{
		for (int i = 1; i < nPlotIndex[iSerieIdx]; i++)
		{
			mpSerie[iSerieIdx].dValueX[i - 1] = mpSerie[iSerieIdx].dValueX[i] - 1;
			mpSerie[iSerieIdx].dValueY[i - 1] = mpSerie[iSerieIdx].dValueY[i];
		}

		nPlotIndex[iSerieIdx] = nPointCount - 1;
	}

	SetXYValue(nPlotIndex[iSerieIdx], y, nPlotIndex[iSerieIdx], iSerieIdx);
